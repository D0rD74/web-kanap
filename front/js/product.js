...
function gen_colors(select, colors){ // Boucle sur l array de couleurs présent dans la requête json et créé de nouveaux enfants à chaque boucle
...
button.addEventListener('click', function(event) {          // On écoute l'événement click
    event.preventDefault();                         // On utilise la fonction preventDefault de notre objet event pour empêcher le comportement par défaut de cet élément lors du clic de la souris
    let quantity = document.getElementById('quantity').value;
    const select = document.getElementById('colors');
    const color = select.options[select.selectedIndex].value;

    addCart(id, quantity, color);
});

function addCart(id, quantity, color){
    let currentCart = getCart();
    let index = currentCart.findIndex((item) => item.id === id && item.color === color);

    if(index == -1) // S'il n'existe pas d'article similaire dans le localStorage
    {
        currentCart.push({
            id : id,
            quantity : quantity,
    ...
    else // Sinon on additionne la quantité à la précédente
    {
